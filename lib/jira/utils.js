...
export function readEnvList(name) {
...
export function mergeLabels(...sources) {
  const set = new Set();
...
export function formatListWithPrefix(text, prefix) {
  if (!text) return '';
...
export function formatUser(user, fallback = undefined) {
  if (!user) {
    return fallback;
  }
...
export function findFieldByName(issue, pattern) {
...
export function buildCleanIssue(issue, cfg) {
  const fields = issue.fields || {};
...
export function formatIssueForLLM(issue, cfg, precomputed) {
...
export function acceptanceBulletToTableRows(text) {
  if (!text) return null;
  const lines = text
    .split(/\r?\n/)
    .map((line) => line.trim())
...
export function formatAcceptanceAsTable(text) {
...
export function parseFields(input, label) {
  if (!input) {
    return {};
  }
...
export function parseFrontMatterMarkdown(filePath) {
...
export function extractJiraOptionsFromMarkdown(filePath) {
