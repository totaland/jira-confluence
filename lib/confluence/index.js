...
function createConfluenceClient() {
  const confluenceModule = require('confluence.js');
...
async function fetchConfluencePage(client, opts) {
  if (opts.id) {
...
async function handleConfluenceRead(opts) {
  const client = createConfluenceClient();
  const page = await fetchConfluencePage(client, opts);
  console.log(JSON.stringify(page, null, 2));
}

async function handleConfluenceUpdate(opts) {
  if (!opts.id) {
...
async function handleConfluenceCreate(opts) {
  const body = readBodyOption(opts);
  if (!body) {
